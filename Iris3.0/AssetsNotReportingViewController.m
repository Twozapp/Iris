//
//  AssetsNotReportingViewController.m
//  Iris3.0
//
...
@interface AssetsNotReportingViewController ()

@end

@implementation AssetsNotReportingViewController
{
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([conf.dashboardSensorView isEqualToNumber:[NSNumber numberWithBool:false]]) {
        self.title = [NSString stringWithFormat:@"%@ not Reporting",conf.gatewayname];
        _assetHeader.text = [NSString stringWithFormat:@"  %@",conf.gatewayname];
    }
    else
    {
        self.title = [NSString stringWithFormat:@"%@ not Reporting",conf.nodename];
